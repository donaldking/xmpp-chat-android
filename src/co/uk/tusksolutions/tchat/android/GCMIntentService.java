...
public class GCMIntentService extends GCMBaseIntentService {
	...
	public void processNotification(JSONObject object) {
		try {
			...
			if (object.getString("messageType").equalsIgnoreCase("CHAT")) {
				...
				groupChatBundle.putString("message", last_message);
				groupChatBundle.putString("messageType", "CHAT_ROOM");
	...
	/*
	 * private void saveNormalChatMessageToDb(Bundle bundle) { /* Insert
	 * received message to db
	 * 
	 * String resource = StringUtils.parseResource(packet.getFrom());
	 * ChatMessagesModel mChatMessageModel = new ChatMessagesModel(); String mid
	 * = packet.getPacketID(); /* If Message from Other Resource and get the
	 * carbon message
	 * 
	 * if (message.getBody().contains("|s|")) {
	 * 
	 * String MessageParse[] = message.getBody().split("\\|s\\|"); String
	 * buddyJID = MessageParse[0]; String last_message = MessageParse[1];
	 * mChatMessageModel.saveMessageToDB(TChatApplication.getCurrentJid(),
	 * buddyJID, resource, StringUtils.parseName(packet.getFrom()),
	 * last_message, 0, "CHAT", System.currentTimeMillis(), 1, mid);
	 * 
	 * } else {
	 * 
	 * mChatMessageModel.saveMessageToDB(TChatApplication.getCurrentJid(),
	 * StringUtils.parseBareAddress(packet.getFrom()), resource,
	 * StringUtils.parseName(packet.getFrom()), message.getBody(), 0, "CHAT",
